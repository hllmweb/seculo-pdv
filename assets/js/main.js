...
//abrir lista de produto 
abrirProduto = (page) =>{
	$("#app-product-list").load(page);
...
recalcular = () =>{
	...
	$(".resultado").val(preco*qtd);
}

//adicionar produto no carrinho
addProdutoCarrinho = (element, page) =>{
	...
	$.ajax({
		...
		success: (data) => {
			fecharModal();
			$("#app-cart-list").load(base_url+"/painel/pagina/listar_produto_carrinho/");
			console.log(data);
		}
...
//deleta produto no carrinho
delProdutoCarrinho = (element, page) =>{
	$.ajax({
		url: page,
		type: 'POST',
		success: (data) => {
...
// 	// if($(".mascara_branca").hasClass("hidden")){
// 	// 	$(".mascara_branca").removeClass("hidden");
// 	// 	$(".mascara_branca").addClass("visible");
// 	// }
